...
int main(int argc, char* argv[]) 
{
  // Convert the args to the integer parameters
  int N = atoi(argv[1]);
  ...
  int i;
  for (i=0; i<2*N; ++i) duration[i] = 1+rand()%5;
#pragma omp parallel shared(N) private(i,t1,t2) num_threads(4)
  {
#pragma omp for schedule(static,bsize) nowait
    for (i=0; i<N; ++i)  {
      // report time of random sleep duration
      t1 = omp_get_wtime();
      sleep(duration[i]);
      t2 = omp_get_wtime();
      printf("Loop 1: Iteration %d: Thread %d, started %e, duration %e\n", 
...
#pragma omp for schedule(static,bsize)
    for (i=0; i<N; ++i)  {
      ...
      sleep(duration[N+i]);
      t2 = omp_get_wtime();
      printf("Loop 2: Iteration %d: Thread %d, started %e, duration %e\n", 
	     i, omp_get_thread_num(), (double)(t1-t0), (double)(t2-t1));
    }
  }
