...
#pragma omp parallel num_threads(4) shared(a,count) private(M,N) reduction(+:sum)
  {
    // initialize the shared variables, only one thread needed
#pragma omp single copyprivate(M,N,sum)
    {
    ...
    // This barrier is inserted to prevent starting the reporting too early
#pragma omp barrier
