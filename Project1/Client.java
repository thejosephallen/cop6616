...
class ClientThread extends Thread{
    ...
    public void run(){
      ...
      // Process result
      // System.out.printf("Thread %d returned: ",this.num);
      if (option == 1){
         // System.out.printf("%d\n",res);
         if (num == 0) result = res; // the first thread alone has correct first
      } else if (option == 2) {
         //thread_num * interval + res // adjust for thread number and array size
         // System.out.printf("%d\n",res);
         if (res != -1) { 
            found_by = num;  
            result = res; // update static with correct result
         }
      } else if (option == 3){
         results[num] = data; // store the sorted array into a static 2D array
       ...
       // this.request_time = (System.currentTimeMillis() - startTime);
       // times.add(this.request_time);
...
public class Client{
   public static void main(String[] args) throws IOException{
      ...
      //for (int i = 0; i < N; i++) data[i] = random.nextInt(65536)-32768;
      Random r = new Random();
      data = r.ints((long) N, 0, 10000).toArray();
      ...
      while(true){
         ...
         else if (menuOp == 3) {data = r.ints((long) N, 0, 10000).toArray();} //rerandomize data
   ...
   public static int[] merge(int[][] lists, int rows, int N){
      ...
      for (int i = 0; i < N; i++){
         ...
         //ystem.arraycopy(lists[min_row], 1, lists[min_row], 0, lists[min_row].length - 1);
      }
      return sorted;
   } 
}