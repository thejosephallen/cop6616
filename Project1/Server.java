...
class ServerThread implements Runnable{
   ...
   //PrintWriter output; // for reading and writing strings
   //BufferedReader input;
   // Process commandProcess; // for reading and writing to server stdin/stdout
   // BufferedReader commandReader;
   //ObjectInputStream obj_in; // for reading and writing objects
   ...
   ServerThread(Socket client){
      try {
         ...
         //this.obj_out = new ObjectOutputStream(client.getOutputStream());
   ...
   public void run(){
      try{         
         ...
         //if (request == 2) System.out.printf("They key to search for is %d",key);
         int len = input.readInt(); // get the size of the data sent
         //System.out.printf("The length of the array in this thread is %d",len);
         int[] data = new int[len]; // initialize an array to hold it

         // Read in the data sent
         for(int i = 0; i < len; i++) data[i] = input.readInt();

         switch (request){
            case 1: // O(1)
               // Acknowledge request
               response = "Received request to return first!";
               System.out.println(response);
               output.writeInt(data[0]);
               break;
            case 2: // O(n)
               // Acknowledge request
               response = "Received request to linear search!";
               System.out.println(response);
               ...
               break;
            case 3: // O(n^2)
               response = "Received request to bubble sort!";
               System.out.println(response);
               ...
               for (int i : data) { 
                  System.out.printf("%d ",i); output.writeInt(i);
               }
               break;
            case 4: // O(n^3)
               response = "Received request to matrix multiply!";
               System.out.println(response);
            default:
         }        
